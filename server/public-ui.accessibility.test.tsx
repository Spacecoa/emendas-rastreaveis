...
describe("acessibilidade das áreas públicas críticas", () => {
  ...
  it("mantém navegação semântica, atalho de conteúdo e ausência de violações axe no contêiner público", async () => {
    const { container } = render(<PortalLayout><h1>Consulta pública</h1><p>Conteúdo verificável.</p></PortalLayout>);
    expect(screen.getByText("Pular para o conteúdo principal").getAttribute("href")).toBe("#conteudo");
  ...
  it("expõe sugestões agrupadas e rótulos associados na busca", async () => {
    ...
    const input = screen.getByLabelText("O que você quer acompanhar?");
