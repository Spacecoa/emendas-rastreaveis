...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    emendas: {
      search: { useQuery: (input: unknown) => { receivedInputs.push(input); return { data: { records: [officialRecord], sourceCoverage: "Carga oficial persistida" }, isLoading: false, error: null }; } },
...
describe("exportação do recorte da busca", () => {
  it("preserva filtros combinados na consulta e exporta apenas o registro retornado", async () => {
    window.history.replaceState({}, "", "/busca?ano=2025&uf=RJ&situacao=informacao_insuficiente&pagoMin=0&autor=GENERAL%20GIRAO&funcao=Defesa%20nacional");
    const createdBlobs: Blob[] = [];
    const createObjectURL = vi.fn((value: Blob) => { createdBlobs.push(value); return "blob:recorte"; });
    ...
    expect(receivedInputs).toContainEqual(expect.objectContaining({ year: 2025, uf: "RJ", status: "informacao_insuficiente", minPaid: 0, author: "GENERAL GIRAO", budgetFunction: "Defesa nacional" }));
    ...
    expect(exported).toEqual([expect.objectContaining({ codigo_emenda: "202539940017", autor: "GENERAL GIRAO", funcao: "Defesa nacional", valor_pago: 39989.35 })]);

    const workbook = XLSX.read(await createdBlobs[2]!.arrayBuffer(), { type: "array" });
