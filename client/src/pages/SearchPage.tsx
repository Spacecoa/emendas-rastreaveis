...
export async function buildExportBlob(
  ...
  const rows = records.map(record => ({
    codigo_emenda: record.code,
    ano: record.year,
    numero_emenda: record.number,
    autor: record.author,
    localidade: record.locality,
    tipo: record.type,
    funcao: record.budgetFunction,
    subfuncao: record.budgetSubfunction,
    valor_empenhado: record.committed,
    valor_liquidado: record.settled,
    valor_pago: record.paid,
    status_cumprimento: record.complianceStatus,
    fonte: record.source,
    url_origem: record.sourceUrl,
    data_extracao: record.extractedAt,
    hash_registro: record.recordHash,
  }));
  let blob: Blob;
  if (type === "json")
    ...
    XLSX.utils.book_append_sheet(book, sheet, "Emendas");
    blob = new Blob([XLSX.write(book, { type: "array", bookType: "xlsx" })], {
...
async function exportRecords(
  ...
  const blob = await buildExportBlob(records, type);
...
export default function SearchPage() {
  ...
  const input = useMemo(() => {
    ...
    return {
      query: params.get("q") ?? "",
      year: Number(params.get("ano") ?? 2025),
      uf: params.get("uf") || undefined,
      status,
      minPaid,
      author: params.get("autor") || undefined,
      budgetFunction: params.get("funcao") || undefined,
      page: 1,
    };
                ...
                {records.length > 0 && (
                  ...
                  >
                    <button
                      ...
                      onClick={() => {
                        void exportRecords(records, "csv");
                      }}
                    >
                    ...
                    <button
                      className="export-button"
                      onClick={() => {
                        void exportRecords(records, "json");
                      }}
                    >
                    ...
                    <button
                      className="export-button"
                      onClick={() => {
                        void exportRecords(records, "xlsx");
                      }}
                    >
                  ...
                  <div className="results-scroll">
                    <table className="min-w-[860px] w-full text-left">
                      <caption className="sr-only">
                        Resultados de emendas parlamentares
                      </caption>
                        ...
                        {records.map(record => (
                          ...
                          >
                            <td className="px-5 py-5">
                              <Link
                                href={`/emendas/${record.code}?ano=${record.year ?? input.year}`}
                                className="font-bold underline decoration-[#1e4a77]/35 underline-offset-4 hover:text-[#1e4a77]"
                              >
                                {record.number
                                  ? `Emenda ${record.number}`
                                  : record.code}
                              </Link>
                            ...
                            <td className="px-5 py-5">
                              <Link
                                href={`/parlamentares/${encodeURIComponent(record.author ?? "sem-autoria")}?ano=${record.year ?? input.year}`}
                                className="font-medium hover:underline"
                              >
                                {record.author ?? "Não informado"}
                              </Link>
                            </td>
                            <td className="px-5 py-5">
                              <Link
                                href={`/municipios/${encodeURIComponent(record.locality ?? "sem-localidade")}?ano=${record.year ?? input.year}`}
                                className="hover:underline"
                              >
                                {record.locality ?? "Não informado"}
                              </Link>
                            </td>
                            ...
                            <td className="px-5 py-5">
                              <Link
                                href={`/emendas/${record.code}?ano=${record.year ?? input.year}`}
                                className="inline-flex size-9 items-center justify-center rounded-full border border-black/15 hover:bg-[#edf4fb]"
                                aria-label={`Ver a emenda ${record.number ?? record.code}`}
                              >
