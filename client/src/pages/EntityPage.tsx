...
export default function EntityPage({ type }: { type: "municipio" | "parlamentar" }) {
  ...
  return <PortalLayout><div className="container py-12 sm:py-16">
    ...
    {search.isLoading ? <div className="mt-10 flex items-center gap-3 rounded-2xl bg-white p-8"><Loader2 className="animate-spin text-[#1e4a77]" /> Consultando registros oficiais…</div> : <>{shown.length === 0 ? <section className="mt-10 rounded-[1.4rem] bg-white p-8 shadow-[0_8px_30px_rgba(18,25,32,.05)]"><h2 className="text-xl font-bold">Nenhum registro oficial corresponde a este recorte.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-black/65">A página não apresenta resultados aproximados de outro município ou parlamentar. Tente a busca pública para ajustar o termo, o ano ou os demais filtros.</p><div className="mt-6"><CompactSearchLink /></div></section> : <>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_.8fr]"><ExecutionBars values={{ committed, settled: shown.reduce((sum, record) => sum + (record.settled ?? 0), 0), paid }} /><aside className="rounded-[1.4rem] border border-dashed border-black/20 bg-[#f8f9fa] p-6"><MapPinned className="text-[#1e4a77]" /><h2 className="mt-5 font-bold tracking-[-.03em]">Mapa e valor por pessoa ainda não publicados</h2><p className="mt-2 text-sm leading-6 text-black/65">Não exibimos estimativas. O mapa e o contexto per capita serão incluídos quando a conciliação com os códigos municipais e a população oficial do IBGE tiverem cobertura e taxa de casamento publicadas.</p></aside></div>
